...
export default class Knapsack {
  ...
  solveZeroOneKnapsackProblem() {
    ...
    // Now let's trace back the knapsack matrix
    // to see what items we're going to add to the knapsack.
    ...
    while (itemIndex > 0) {
      ...
      if (
        ...
        const pervPrevSumValue = knapsackMatrix[itemIndex - 2][weightIndex];

        if (!prevSumValue || (prevSumValue && pervPrevSumValue !== prevSumValue)) {
